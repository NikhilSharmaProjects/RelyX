...
import { Plus, Minus, Shield, Download, FileQuestion, Mail, CheckCircle2, ArrowRight } from 'lucide-react';
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-2">
        ...
        <span className="text-2xl font-extrabold tracking-tight text-relyx-500">RelyX</span>
      </div>
      <div className="hidden md:flex items-center gap-8 font-medium text-relyx-400">
        <a href="#features" className="hover:text-relyx-200 transition-colors">Features</a>
        <a href="#compare" className="hover:text-relyx-200 transition-colors">Compare</a>
        <a href="#faq" className="hover:text-relyx-200 transition-colors">FAQ</a>
      </div>
    </nav>
  );
}

function FeatureCard({ title, icon, delay }: { title: string, icon: React.ReactNode, delay: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <h3 className="text-2xl font-bold text-relyx-500 mb-4 leading-tight">{title}</h3>
        ...
        {isOpen && (
          ...
          >
            <p className="pt-6 pl-14 text-lg text-gray-600 leading-relaxed">
              This is a placeholder answer for the FAQ. RelyX is designed to keep you safe while browsing by detecting and blocking malicious content before it can harm your device or steal your data.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function App() {
  const features = [
    { title: "URL detection", icon: <Shield className="text-relyx-200 w-8 h-8" /> },
    { title: "Malicious download detection", icon: <Download className="text-relyx-200 w-8 h-8" /> },
    { title: "Fake form detection", icon: <FileQuestion className="text-relyx-200 w-8 h-8" /> },
    { title: "Fake email and phishing attacks detection", icon: <Mail className="text-relyx-200 w-8 h-8" /> }
  ];
    ...
    <div className="min-h-screen bg-relyx-100 selection:bg-relyx-200 selection:text-white">
      ...
      <div className="relative overflow-hidden bg-slate-50/50">
        ...
        <section className="relative pt-24 pb-32 px-6 text-center max-w-5xl mx-auto z-10">
          ...
          >
            The extension that put <br className="hidden md:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-relyx-300 to-relyx-200">safety first</span>
          </motion.h1>
      ...
      <section id="features" className="py-32 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((f, i) => (
              <FeatureCard key={i} title={f.title} icon={f.icon} delay={i * 0.1} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="compare" className="py-32 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {comparisons.map((c, i) => (
              <CompareCard key={i} title={c.title} delay={i * 0.1} />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          >
            <div className="flex flex-col items-center gap-4">
              ...
              <span className="text-relyx-200/80 text-2xl font-medium">Screenshot Placeholder</span>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16">
            <div className="flex items-center gap-3">
              ...
              <span className="text-3xl font-extrabold tracking-tight">RelyX</span>
            </div>
          ...
          <div className="border-t border-relyx-400/50 pt-8 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-relyx-200/60 text-lg">&copy; 2026 RelyX. All rights reserved.</p>
            <p className="text-relyx-200/40">The extension that put safety first.</p>
          </div>
        </div>
      </footer>
    </div>
